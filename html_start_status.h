const char START_STATUS_HTML[] PROGMEM = R"rawliteral(
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .peer-dot.stale {
      background: var(--mass-gold);
      box-shadow: 0 0 4px var(--mass-gold);
    }
    .peer-dot.offline {
      background: var(--mass-red);
    }
    ...
    .peer-role.finish { background: rgba(74,144,217,0.2); color: var(--mass-blue); }
    .peer-role.start { background: rgba(40,167,69,0.2); color: var(--mass-green); }
    .peer-role.speedtrap { background: rgba(212,175,55,0.2); color: var(--mass-gold); }
  </style>
</head>
<body>
  <div class="container">
    ...
    <!-- Brother's Six — Peer Network -->
    <div class="card" id="peerCard">
      <div class="card-title">Brother's Six — Peer Network</div>
      <div id="peerList">
        <div class="info-row">
          <span class="info-label">Scanning...</span>
          <span class="info-value" style="color:var(--mass-muted);">Searching for peers</span>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    // ====================================================================
    // PEER NETWORK STATUS (polled)
    // ====================================================================
    async function loadPeers() {
      try {
        var resp = await fetch('/api/peers');
        ...
        if (!peers || peers.length === 0) {
          container.innerHTML = '<div class="info-row">' +
            ...
            '<span class="info-value" style="color:var(--mass-muted);">Beaconing...</span></div>';
          return;
        }
        ...
        for (var i = 0; i < peers.length; i++) {
          ...
          var statusClass = p.status === 'online' ? 'connected' :
                           p.status === 'stale' ? '' : 'disconnected';
